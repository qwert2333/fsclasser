...
void PFAFastSimu::init() {
	...
	_outputTree->Branch("Num", &_nEvt, "Num/I");
	_outputTree->Branch("MCP", _MCP, "MCP[3]/D");
	_outputTree->Branch("MCPEn", &_MCPEn, "MCPEn/D");
	_outputTree->Branch("MCPID", &_MCPID, "MCPID/I");
	_outputTree->Branch("Eff", &_Eff, "Eff/D");
	_outputTree->Branch("FlatRnd", &_FlatRnd, "FlatRnd/D");
	_outputTree->Branch("SF", &_SF, "SF/D");	//Scale Factor
	_outputTree->Branch("ealpha", ealpha, "elapha[10]/D");	//Scale Factor
	*/
	ifstream electron("/home/bes/lig/pub/workfs/higgs/FSClasser/parameter/electron.para");
	for(int i=0; i<10; i++)
	{
	...
	ifstream muon("/workfs/bes/lig/higgs/FSClasser/parameter/result.txt");
	...
	ifstream pion("/home/bes/lig/pub/workfs/higgs/FSClasser/parameter/pion.para");
	for(int i=0; i<10; i++)
	{
		for(int j=0; j<19; j++)
	...
	{		
		...
		{    
			...
			for(int i0 = 0; i0 < _nMCP; i0++)
			{
				...
				if ( fabs(costheta)>0.99500  ) continue;	
				if(NParent == 0)
				{
