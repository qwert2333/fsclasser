...
void PhotonSelectionProcessor::processEvent( LCEvent * evt ) { 
	...
	Double_t fMassHCut = 20.;             // mass cut for photon pair from H
	...
	if (!hMc) {
		...
		tupstr_mc 
			<< "id:pdg:motherpdg:mass:nparents:energy:ndaughters:original:daughterpdg" << ":"
			<< "charge"
			<< ends;
	...
	//Int_t nWqq = 0, nWlv = 0, nWev = 0, nWmv = 0; // tag W decay
